...
import org.spagic3.components.drools.activator.DroolsComponentActivator;
...
public class OSGiServiceInvoker extends AbstractSpagicService implements IServiceInvoker{
	...
	public void invokeService(String serviceID, Exchange exchange) {
		...
		try{
			storedExchanges.put(exchangeId, exchange);
			logger.info(" Storing exchange ["+exchangeId+"] Associated to ["+this.getSpagicId()+"] -> ["+serviceID+"]");
			...
			storedExchanges.remove(exchangeId);
	...
	public void invokeSync(String serviceID, Exchange exchange) {
		...
		try {
			...
			refs = DroolsComponentActivator.getCtx().getServiceReferences(
		...
		if (refs[0] != null) {
			service = (ISpagicService) DroolsComponentActivator.getCtx()
					.getService(refs[0]);
			...
			if (service != null) {
				try {
					...
					DroolsComponentActivator.getCtx().ungetService(refs[0]);
				}
			} else {

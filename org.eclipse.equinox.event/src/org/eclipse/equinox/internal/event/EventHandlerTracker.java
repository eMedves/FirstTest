...
public class EventHandlerTracker extends ServiceTracker implements EventDispatcher {
	...
	private int defaultMaxThreadPoolSize = 100;
	private static final String EA_MAX_THREAD_POOL_SIZE = "ea.maxThreadPoolSize";
	// -- AZ End
	...
	public EventHandlerTracker(BundleContext context, LogService log) {
		...
		// -- AZ Start
		String threadPoolSize = System.getProperty(EA_MAX_THREAD_POOL_SIZE);
		int threadPoolSizeInt = (threadPoolSize != null ? Integer.valueOf(threadPoolSize).intValue() : defaultMaxThreadPoolSize);
		threadExecutor = Executors.newFixedThreadPool(threadPoolSizeInt);
		// -- AZ End
	}
	...
	public void dispatchEvent(Object eventListener, Object listenerObject, int eventAction, Object eventObject) {
		...
		// -- AZ Start
		Runnable runnable = new EventHandlerTrackerThread((EventHandlerWrapper) eventListener, 
												(Event) eventObject, 
												(Permission) listenerObject);

...
public class EDIX12DataCompletion extends DataCompletion {
	...
	private void completeInterchange() {
		...
		conservativeSetValue(FI08, SchemaDate.now().toString());
		ITextNode FI09 = MakeSureHasChild.at(isa, "FI09", 9);
		conservativeSetValue(FI09, SchemaTime.now().toString());
	...
	private void completeGroup() {
		...
		if (!GS.isNull()) {
			...
			conservativeSetValue(GSF373, SchemaDate.now().toString());
			ITextNode GSF337 = MakeSureHasChild.at(GS, "F337", 4);
			conservativeSetValue(GSF337, SchemaTime.now().toString());

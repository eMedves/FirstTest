...
public class JBFHelper {
	...
	private boolean ignoreFaultStrings = false;
	...
	public int openSession() {
		try {
			...
			SOAPMessage response = call(this.properties.getProperty(JBFConstants.JBF_URL),
					jbfSOAPCallRequest);
			...
			if (isBalanced()) {
				...
				// NB. se dovranno essere passati piu coockie, far diventare
				// cookieOpenSession come String [], e usare il for each:
				// for(String fullCookieStr :
				// response.getMimeHeaders().getHeader("Set-Cookie") )
				// System.out.println("\nGLC: .... : " + fullCookieStr);
				// this.lblSessionId =
				// response.getMimeHeaders().getHeader("lblSessionId")[0];
			}
			...
			if (isErrorResponse(body)) {
				...
				// this.lblSessionId = retrieveLBLSessionId(body); //GLC non
				// serve lato app
	...
	public int callWS() {
		try {
			...
			SOAPMessage jbfSOAPCallRequest = createJBFSoapMessageRequest(payloadDoc
					.getRootElement());
			SOAPMessage response = call(this.properties.getProperty(JBFConstants.JBF_URL),
					jbfSOAPCallRequest);
	...
	public int closeSession() {
		try {
			...
			// GLC serve ache per la chiusura? viene aggiunto nella
			// createJBFSoapMessageRequest
	 ...
	 * Document bodyDocument = DocumentHelper.parseText(body); org.dom4j.Node
	 * callret = bodyDocument.selectSingleNode("/callResponse/callReturn");
	 * String callRetText = callret.getText();
	 * 
	 * Document callRetDoc = DocumentHelper.parseText(callRetText);
	 * org.dom4j.Node lblSessIdDoc =
	 * callRetDoc.selectSingleNode("/Output/OpenSession/lblSessionID");
	 * 
	...
	public static SOAPMessage call(String wsEndpoint, SOAPMessage soapRequest) throws SOAPException {
		...
		try {
			SOAPConnectionFactory soapConnectionFactory = javax.xml.soap.SOAPConnectionFactory
					.newInstance();
